...
from reframe.server.api.agents import single_action_chat_agent
from loguru import logger
from fastapi import APIRouter, Depends, HTTPException, Request, Form, Body, UploadFile, status
from pprint import pprint, pformat

# Internal Libraries
from reframe.server.lib.db_models.dataframe import Blueprint, DATA_TYPE, Dataframe, HistoryItem
from reframe.server.lib.security import get_api_key
from reframe.server.lib.db_models.namespace import Namespace
from reframe.server.lib import hasura
from reframe.server.lib.util import flatten_prompt, extract_prompt_input_column_recursive
...
async def dataframe_upload(request: Request, namespace: Annotated[Namespace, Depends(get_api_key)], prompt: Prompt):
    ...
    if prompt.prompt_version == 'v1.0':
        prompt.input_column = extract_prompt_input_column_recursive(prompt.prompt, trigger='@')
    ...
    if prompt.input_column is None:
        raise HTTPException
    ...
    if limit is not None:
        sql_text_query = f"""
        SELECT _id, {prompt.input_column} FROM {dataframe.table_name}
        LIMIT {limit}
        """
    elif len(id_list) > 0:
        sql_text_query = f"""
