...
from reframe.server.lib import sql_text, hasura
...
async def create_namespace(request: Request, namespace: Namespace):
    ...
    try:
        ...
        data_db_name = f"reframe_datadb_{namespace_short_id}_{namespace.slug}"
        trace_db_name = f"reframe_tracedb_{namespace_short_id}_{namespace.slug}"

        namespace.data_db_params = {
            "host": os_env.get("REFRAME_DATA_DB_HOST"),
            "database": data_db_name,
            "password": os_env.get("REFRAME_DATA_DB_PASS"),
            "user": os_env.get("REFRAME_DATA_DB_USER"),
            "port": os_env.get("REFRAME_DATA_DB_PORT")
        }
        namespace.trace_db_params = {
            "host": os_env.get("REFRAME_TRACE_DB_HOST"),
            "database": trace_db_name,
            "password": os_env.get("REFRAME_TRACE_DB_PASS"),
            "user": os_env.get("REFRAME_DATA_DB_USER"),
            "port": os_env.get("REFRAME_DATA_DB_PORT")
        }
        ...
        # Create the data database and grant privileges to the reframe user.
        ...
        # Create the trace database and grant privileges to the reframe user.
        await request.app.state.meta_db.execute(
